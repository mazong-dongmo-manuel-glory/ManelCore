...
def _load_rich_context() -> dict[str, Any]:
    ...
    try:
        with Neo4jConnection() as conn:
            ...
            # Recent opportunities to avoid duplicate searches
            ...
            ctx["recent_opportunities"] = [
                {"titre": o.get("titre", ""), "source": o.get("source", "")}
                for o in recent
            ]
    except Exception:
        pass
...
def _opportunity_match_key(opp: dict[str, Any]) -> str:
    for key in ("url", "seao_uuid", "reference_id", "numero", "titre", "title"):
        value = opp.get(key)
        if value:
...
async def rank_and_analyze(state: ExplorerState) -> Dict[str, Any]:
    ...
    for opp in opportunities:
        key = opp.get("url") or opp.get("titre", "")
        if key and key not in seen:
            seen.add(key)
            unique.append(opp)
    ...
    async def process_batch(batch: list[dict]) -> list[dict] | None:
        ...
        prompt = (
            ...
            '- "score_pertinence": float de 0.0 à 1.0. (0.9+ = match parfait, <0.5 = hors sujet). SOIS STRICT ET PRÉCIS. Ne donne jamais un score générique de 0.5 par défaut. Évalue réellement chaque opportunité (ex: 0.2, 0.85, 0.95).\n'
            ...
            "  * **Adéquation Secteur (X%)**: Explique le lien direct avec les secteurs/services de l'entreprise.\n"
            "  * **Analyse Technique**: Les exigences clés du donneur d'ordre et notre capacité à y répondre.\n"
            "  * **Recommandation**: Pourquoi ManelCore devrait (ou non) soumissionner.\n\n"
        ...
        with Neo4jConnection() as conn:
            ...
            for opp in ranked:
                try:
                    ...
                    # Filtrer : On ne garde pas les opportunités dont le score est <= 0.5
                    if score <= 0.5:
                        continue
                        
                    repo.upsert_opportunite({
                        "titre":            opp.get("titre") or opp.get("title") or "Sans titre",
                        "source":           opp.get("source", ""),
                        "url":              opp.get("url", ""),
                        "statut":           "nouveau",  # Changé de 'en_attente' à 'nouveau' pour l'UI
                        "type":             opp.get("type", ""),
                        "date_publication": str(opp.get("date_publication") or opp.get("date") or ""),
                        "date_limite":      str(opp.get("date_limite", "")),
                    ...
                    # Engagement automatique si un contact est présent et pertinence élevée
                    email = opp.get("contact_email")
                    if email and float(opp.get("score_pertinence", 0)) > 0.7:
                        try:
                            # On délègue l'engagement à l'API (via BackgroundTask ou appel direct)
                            # Pour simplifier ici, on enregistre juste l'intention dans les logs
                            # L'API main.py ou un service dédié s'occupera de la notification Telegram
                            logger.info(f"Engagement automatique suggéré pour {email}")
                            await _emit_live_event("Système", f"Contact suggéré pour l'entreprise: {opp.get('organisation', 'Inconnue')} ({email})")
                        except Exception as e:
                            logger.error(f"Auto-engagement error: {e}")
